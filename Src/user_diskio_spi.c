...
//If you want to use a different SPI port change hspi3 here and elsewhere in this file
extern SPI_HandleTypeDef hspi3;
#define HSPI_SD hspi3

/* Function prototypes */

#define FCLK_SLOW() { \
	__HAL_SPI_DISABLE(&HSPI_SD); \
	uint16_t nReg = READ_REG(HSPI_SD.Instance->CR1); \
	nReg &= ~ (7<<3); \
	nReg |= SPI_BAUDRATEPRESCALER_128; \
	WRITE_REG(HSPI_SD.Instance->CR1, nReg ); \
	__HAL_SPI_ENABLE(&HSPI_SD); \
}
#define FCLK_FAST() { \
	__HAL_SPI_DISABLE(&HSPI_SD);	\
	uint16_t nReg = READ_REG(HSPI_SD.Instance->CR1);	\
...
{
	BYTE rxDat;
    HAL_SPI_TransmitReceive(&HSPI_SD, &dat, &rxDat, 1, 50);
    return rxDat;
}
...
//XXX is this the way?
	//GPIO_PIN_RESET
	//HAL_GPIO_ReadPin(MICRO_SD_CARD_INSERTED_GPIO_Port, MICRO_SD_CARD_INSERTED_Pin);
